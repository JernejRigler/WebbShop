    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `/api/izdelki/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}`
        );
            ...
            <ul>
              {ratings.map((r) => (
                <li key={r.name}>
                  ...
                  >
                    <Ocena naziv={' & up'} ocena={r.rating}></Ocena>
                  </Link>
                </li>
              ))}
              <li>
        ...
        <Col md={9}>
          {loading ? (
            ...
            <>
              ...
              {products.length === 0 && <Sporocilo>No Product Found</Sporocilo>}

              <Row>
                {products.map((product) => (
                  <Col sm={6} lg={4} className="mb-3" key={product._id}>
                    <Izdelek izdelek={product}></Izdelek>
                  </Col>
                ))}
              </Row>
